...
typedef long long int td; typedef vector<int> vi; typedef vector<td> vd;//Don't forget to change de td
const td INF=1e18;//for maximum set INF to 0, and negate costs
bool zero(td x){return abs(x)==0;}//change to x==0, for ints/ll
struct Hungarian{
...
int main() {_ 
    ...
    while(pos<n){
        ...
        while(pos2<n){
            lli current =0;
            if(pos2>0){
                current+=road[pos2-1][pos];
            }
            if(pos2+1<n){
                current+=road[pos][pos2+1];
            }
            h.set(cont,cont2,current);
            cont2++;
            pos2+=2;
        }
        cont++;
